...
#### Disk Usage
6. Очень длинная команда за счет конвейера,
...
буквы `о`, какой-то `возможный раздел-rwxr----x 1 harry users 2.7K Jan 4 07:32 /home/karpaff/linuxtutorialwork/chick.pngитель` и в конце входящее или
...
29. Скачаем датасет с Kaggle при помощи `kaggle API` и поработаем grep'ом в нем. 
    1. Сперва установим
    утилиту через CLI, используя пакетный менеджер: `pip install kaggle`.
    Если после установки у Вас не работает команда `kaggle` в CLI, то в переменную
    окружения нужно добавить путь к исполняемому файлу `kaggle`. Затем нужно
    авторизоваться, чтобы пользоваться API. В вашем профиле на сайте скачиваем
    сформировавшийся ключ и перемещаем его в `~/.kaggle/`. Done! 
    Установка датасета: `kaggle datasets download -d kaggle/hillary-clinton-emails`.
    
    2. Найдем все email-адреса, которые записаны в `Email.csv`, и выведем
    их в отсортированном формате, оставив только уникальные записи. Путем нехитрых
    раздумий было выяснено, что все email-ы заключены в `<>`, поэтому воспользуемся
    `lookaround` в нашем паттерне. Итоговый результат будет выглядеть так:
    
```shell
grep -P -o "(?<=\<)([a-zA-Z0-9._-]+@[a-zA-Z0-9._-]+\.[a-zA-Z0-9_-]+)(?=\>)" \
      Emails.csv | sort -u
```


30. В п.29 команда получилось чересчур длинной, поэтому добавим регулярное
выражение в файл.

```shell
touch searchReg.txt
echo "(?<=\<)...(?=\>)" > searchReg.txt # сократил для вместимости
grep -P -f searchReg.txt Emails.csv | sort -u
```
...
в Linux. Так например [терминальный калькулятор](terminal-calculator), который
лежит уже в скомпилированном виде, можно использовать как классическую
утилиту в операционной системе. Добавим ссылку на исполняемый файл, чтобы иметь
...
```shell
user@:~$ terminal-calculator 

>> 1 + 1
<< ... = 2.000000 + 0.000000j
>> 10 + a * 5 - 10*j
a? >> 5
<< ... = 35.000000 + -10.000000j
>> exp(sqrt(PI*j))-sin(real(10*j+10))
<< ... = 1.637238 + 3.326919j
>> exit
...
50. [Python-скрипт](./weather.py), определяющий по IP
